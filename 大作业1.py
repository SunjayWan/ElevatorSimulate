...
class Passenger():
    ...
    def __init__(self, lift):
        self._start = r.randint(1,n)
        self._end = self._genend()
        self.direction = lambda start,end: -1 if start > end else 1 (start,end)
        self._lift = lift
        self._order
        self._arrivetime = classmethod._gentime().next()
...
class Lift():
    def __init__(self, speed, lmax):
        ...
        self._speed = speed
        self._max = lmax
        self._passengerquene = PrioQuene()
        self._orderquene = Quene()
...
def simulift():
    ...
    allpassenger = genpassenger(time)
